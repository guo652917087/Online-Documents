...
1. [产品介绍](#产品介绍)
	...
	- [1.3 硬件版本说明](#硬件版本说明)
		- [1.3.1 CN470 全双工版本](#cn470-全双工版本)
		- [1.3.2 EU868 LBT 版本](#eu868-lbt-版本)
		- [1.3.3 US915/AS923 LBT 版本](#us915as923-lbt-版本)
	- [1.4 软件功能特性](#软件功能特性)
	- [1.5 系统架构](#系统架构)
	- [1.6 产品竞争力总结](#产品竞争力总结)	
...
3. [网关页面操作指引](#网关页面操作指引)
	- [3.1 登录网关页面](#登录网关页面)
		- [3.1.1 使用WiFi登录网关页面](#使用wifi登录网关页面)
		- [3.1.2 使用LAN口网线登录网关页面](#使用lan登录网关页面)
	...
	- [3.2 网关上网方式](#网关上网方式)
		...
		- [3.2.4 使用2.4G WiFi方式连接路由器](#使用24g-wifi方式连接路由器)
		- [3.2.5 使用5.8G WiFi方式连接路由器](#使用58g-wifi方式连接路由器)
...
IMX93-GW8016 是一款专业的工业级 LoRaWAN 边缘网关产品，可室外安装，采用高性能 NXP i.MX93 三核处理器，集成双 SX1302 射频芯片，提供 16 个 LoRa 上行通道和 2 个下行通道。该网关专为大规模物联网部署设计，内置 chirpstack V4.x 网络服务器，支持多种通信协议和数据转发方式，可满足智慧城市、智慧农业、工业物联网等多种应用场景需求。
...
- 提供墙面和抱杆安装套件
- 内置 GPS/北斗/Galileo/GLONASS 多模定位
- 支持 VPN 远程访问，便于远程维护和技术支持

### 8. 强大的二次开发能力
- Python 3.11 脚本支持，可编写自定义数据处理逻辑
- IoT Hub 支持 物模型TSL (Thing Specification Language)定义，灵活配置设备属性
- 支持集群管理和设备漫游功能
...
| ------ | ---------------- | --------------- |
| 型号   | GW8016 | GW8000 |
| CPU | NXP i.MX93 三核处理器 | NXP i.MX93 三核处理器 |
...
| LoRa 主芯片 | SX1302 / SX1303 / SX1301 | SX1302 / SX1303 / SX1301 |
| LoRa 工作频段 | CN470-510 / EU863-870 / US902-928 / AS923-1 / AS923-2 / AS923-3 / AS923-4 / AU915-928 / KR920-923 / RU864-870 / IN865-867 | 同左 |
| LoRa 通信速率 | 292 bps ~ 5.4 kbps，支持扩频因子 SF7~SF12（SX1302/SX1303 支持 SF5/SF6） | 同左 |
...
| LoRa 底噪扫描 | SX1302 方案支持 | SX1302 方案支持 |
| LoRa 天线 | 2 根 5 dBi 玻璃钢天线 | 1 根 5 dBi 玻璃钢天线 |
| LoRa 天线类型 | 全向 | 全向 |
| LoRa LBT | SX1302 方案支持 | SX1302 方案支持 |
...
| Wi-Fi | 2.4G / 5.8G AP 模式，2×2 MIMO，支持 STA 连接路由器 | 同左 |
| BT | BLE 5.0 以上 | BLE 5.0 以上 |
| 天线接口 | 5 个 | 4 个 |
...
| 防雷 | 符合 GB50343-2004 中规定的 B 级雷电防护等级 | 符合 GB50343-2004 中规定的 B 级雷电防护等级 |
...
### 1. IMX93-2-470F-x.x.x(CN470 全双工版本)
...
### 2. IMX93-2-868-x.x.x(EU868 LBT 版本)
...
### 3. IMX93-2-915-x.x.x(US915/AS923 LBT 版本)

**适用地区**：美国、澳大利亚、日本、韩国、东南亚等地区
...
**chirpstack 进程功能**：
- 完整的 LoRaWAN 网络服务器（V4.x），内置在网关中
- 通过 ZMQ `ROUTER/DEALER` 与 lora 进程通信，实现 concentratord 协议
- 核心功能：
  - 设备入网管理（OTAA/ABP）
  - LoRaWAN 帧解析和验证
  - 设备会话管理和数据解密
...
**PostgreSQL 数据库**：
- **chirpstack 数据库**：设备信息、应用配置、网关注册、帧日志
- **iot 数据库**：
...
**Redis 缓存**：
- chirpstack 会话数据：DevAddr、SessionKeys、帧计数器
- 设备状态缓存：最后上线时间、电池电量、信号强度
- 集群数据同步：多网关间共享设备会话，实现无缝漫游
...
- **全双工支持**：CN470 频段全双工模式，接收灵敏度达 -138 dBm
...
| **内置 NS** | ✅ ChirpStack V4.x 全功能（定期更新到最新版） | ⚠️ 无或功能受限的旧版本 |
| **LBT 支持** | ✅ EU868/AS923/KR920 硬件级支持 | ❌ 大多数不支持或软件模拟 |
| **历史数据查询** | ✅ 按时间段/条数查询，Excel 导出 | ❌ 大多数不支持 |
| **离线数据缓存** | ✅ 网络恢复后自动续传 | ❌ 数据丢失 |
| **FUOTA 固件升级** | ✅ 支持 LDPC 算法，20-30% 丢包容忍 | ❌ 大多数不支持 |
| **Modbus TCP** | ✅ 支持完整映射 | ❌ 不支持 |
| **BACnet BIP** | ✅ 支持完整映射 | ❌ 不支持 |
| **IoT Hub 物模型** | ✅ 自由定义 TSL 物模型 | ❌ 不支持或固定模型 |
| **集群管理** | ✅ 去中心化边缘集群，设备漫游 | ⚠️ 依赖中心化云端 |
| **远程协助** | ✅ 加密 VPN，无需第三方工具 | ⚠️ 依赖 TeamViewer 等工具 |
| **Python 脚本** | ✅ Python 3.11 完整支持 | ❌ 不支持 |
| **Node-RED** | ✅ 集成 Node-RED（通过 ChirpStack） | ❌ 不支持 |
...
1. **全栈 ChirpStack**：提供完整的 ChirpStack V4.x 功能，而非阉割版或旧版本
2. **LBT 硬件支持**：满足严格的监管要求，提升频谱利用效率
3. **数据零丢失**：离线缓存 + 自动续传机制
4. **远程运维**：FUOTA + VPN 远程协助，降低运维成本
5. **工业协议集成**：Modbus TCP + BACnet BIP，无缝对接现有系统
6. **边缘计算**：去中心化集群，无需依赖云端
...
> **注意**：烧录过程中请勿断电，否则可能导致设备损坏

**烧录模式进入方法**：
1. 断开网关电源
2. 长按"烧录按钮"（位于设备侧面，标记为RECOVERY）
3. 保持按住的同时给网关上电
4. 等待LED指示灯闪烁后松开按钮
5. 此时网关已进入烧录模式
...
**所需工具**：
- TF卡（Micro SD卡，建议8GB以上，Class 10）
- 网关固件文件（已制作成TF卡启动镜像）

**操作步骤**：
1. 使用工具将固件镜像写入TF卡（使用balenaEtcher或dd命令）
2. 将TF卡插入网关的TF卡槽
3. 网关上电，自动从TF卡启动并烧录固件
4. 烧录完成后，网关自动重启（LED指示灯会闪烁提示）
5. 取出TF卡，网关从eMMC正常启动

> **提示**：
> - TF卡烧录方式无需连接电脑，适合现场快速恢复固件
> - 如果TF卡已插入但网关未自动从TF卡启动，请检查TF卡是否写入正确的启动镜像
> - 首次烧录可能需要5-10分钟，请耐心等待
...
```bash
# 使用dd命令写入
sudo dd if=imx93-gw8016-firmware.wic of=/dev/sdX bs=4M status=progress
sudo sync
```
...
- 网关已正常运行
- 已登录网关管理页面
- 已下载最新固件文件（.tar 或 .img.gz 格式）

**操作步骤**：
1. 登录网关管理页面：`http://192.168.3.1` 或通过WAN口IP访问
2. 进入 **系统** > **备份/升级** 页面
3. 在"升级固件"部分，点击"选择文件"按钮
4. 选择下载的固件文件
5. 点击"上传"按钮，等待上传完成
6. 系统自动校验固件，确认无误后点击"执行升级"
...
2. 电脑会自动从网关获取 192.168.3.x 网段的 IP 地址（网关内置 DHCP 服务器）
3. 打开浏览器，访问网关管理地址：`http://192.168.3.1`
4. 如已配置 HTTPS，可使用：`https://192.168.3.1`
...
网关开机默认会创建5.8G的 WiFi AP热点(如果配置WiFi去连路由器(STA客户端)，那么没有热点(AP模式)，GW8000的WiFi AP模式和STA客户端模式只能二选一，与GW1000网关的WiFi不一样)，便于无线接入配置。
...
2. 在电脑或手机的 WiFi 列表中查找以下格式的热点：
   ...
   - 示例：`WiFi-10_A1B2C3`
3. 连接该 WiFi 热点（首次连接无需密码，或使用默认密码）
4. 连接成功后，浏览器访问：`http://192.168.3.1`

**登录信息：**
- 用户名：`root`
- 默认密码：`WelcomeTo2018`

> **提示**：网关 ID 可在设备铭牌或包装标签上找到。
...
**前提条件：**
- 网关 WAN 口已连接到路由器或交换机
- 已知网关获取的 WAN IP 地址（可通过路由器 DHCP 客户端列表查询，或通过串口/LAN 口登录后查看）
...
**操作步骤**：
1. 打开浏览器，访问：`http://192.168.3.1:8080`（LAN/WiFi 访问）
...
网关基于 OpenWrt-24.10 系统，提供了强大的 Web 管理界面，涵盖 LoRa 网络管理、设备监控、数据处理、远程运维等全方位功能模块。
...
**核心功能**：
- **实时状态监控**：显示 4G 模块状态、信号强度（RSSI、SINR）、网络类型（LTE/3G/2G）、运营商信息
- **连接管理**：支持 APN、认证类型（PAP/CHAP/None）、用户名/密码配置
- **智能协议选择**：支持 IPv4/IPv6 双栈，可根据网络环境自动切换
- **网络诊断**：实时显示连接状态、IP 地址、DNS 服务器、数据流量统计
...
**功能位置**：`www/gateway cluster` 目录

**核心功能**：
- **集群配置**：支持多网关组网，实现负载均衡和设备漫游
- **会话同步**：LoRaWAN 设备会话密钥在集群内自动同步
- **集群状态监控**：实时查看集群内各网关状态、在线设备数量、负载分布
- **冗余备份**：主备网关自动切换，确保服务高可用

**技术优势**：
- **零配置漫游**：设备在网关覆盖范围内移动时，无需重新入网
- **会话密钥共享**：基于 Redis 或数据库实现会话密钥同步
- **负载均衡**：智能分配设备连接，避免单点过载
- **故障自愈**：网关故障时自动切换到备用网关
...
**核心功能**：
- **多系统定位**：支持 GPS、北斗、Galileo、GLONASS 多系统联合定位
- **实时位置显示**：Web 界面实时显示经纬度、海拔、定位精度、卫星数量
- **轨迹记录**：支持位置历史记录，可导出轨迹数据
- **时间同步**：GPS 授时，为 LoRaWAN 下行提供高精度时间基准

**技术优势**：
- **高精度定位**：多系统联合定位，精度可达 2.5 米（CEP）
- **快速冷启动**：首次定位时间 < 30 秒
- **全球覆盖**：支持全球任意地点定位
- **授时精度**：GPS 授时精度 < 1 微秒
...
**功能位置**：`www/hub config` 目录

**核心功能**：
- **物模型定义**：基于 物模型TSL (Thing Specification Language) 标准，自由定义传感器属性、事件、服务
- **属性管理**：支持多种数据类型（整型、浮点型、字符串、布尔型等）
- **协议映射**：物模型TSL (Thing Specification Language) 属性自动映射到 Modbus 寄存器、BACnet 对象
- **设备模板**：预定义常见传感器模板，快速添加设备

**技术优势**：
- **统一数据模型**：一次定义，多协议复用
- **灵活扩展**：支持自定义属性字段
- **语义化描述**：属性名称、单位、描述清晰
- **零代码配置**：通过 Web 界面配置，无需编程

---

### 5. Hub 历史数据查询与导出

**功能位置**：`www/hub history` 目录

**核心功能**：
- **时间范围查询**：支持按时间段查询设备历史数据
- **多维度筛选**：按设备 EUI、设备名称、应用 ID、数据类型筛选
- **数据可视化**：时间序列图表展示，支持多设备数据对比
- **Excel 导出**：一键导出历史数据为 Excel 表格

**技术优势**：
- **高效查询**：支持千万级数据快速检索
- **灵活导出**：Excel 格式兼容性强
- **数据完整性**：包含原始 LoRaWAN 帧和解析后的业务数据
- **离线分析**：支持数据下载后离线分析
...
**功能位置**：`www/hub status` 目录

**核心功能**：
- **设备在线状态**：实时显示所有设备在线/离线状态
- **实时数据展示**：根据物模型定义，实时显示设备属性值
- **数据刷新**：支持自动刷新（5 秒/10 秒/30 秒可配置）
- **阈值告警**：设置属性阈值，超限时高亮显示并推送告警

**技术优势**：
- **轻量级实时推送**：延迟 < 3 秒
- **大规模设备支持**：单网关支持显示 1000+ 设备状态
- **可视化仪表盘**：直观显示设备健康状态
...
**功能位置**：`www/lora config` 目录

**核心功能**：
- **多协议切换**：UDP GWMP、MQTT、chirpstack-mqtt-forwarder、Basic Station LNS/CUPS
- **频段与信道配置**：支持 CN470、EU868、US915、AS923 等全球频段
- **全双工/半双工切换**：CN470 频段支持全双工模式配置
- **LBT 配置**：EU868、AS923、KR920 频段支持 LBT 功能
- **证书管理**：上传/管理 TLS 证书

**技术优势**：
- **零代码切换**：通过 Web 界面一键切换协议
- **协议兼容性强**：支持所有主流 LoRaWAN 协议和云平台
- **灵活部署**：同一网关可同时运行双协议
- **安全加固**：支持 TLS 双向认证

---

### 8. LoRa 底噪扫描（Spectrum Scan）

**功能位置**：`www/lora scan` 目录

**核心功能**：
- **频谱扫描**：扫描 LoRa 工作频段内的 RF 信号强度（RSSI）
- **干扰检测**：识别同频干扰源
- **信道质量评估**：评估每个信道的噪声水平
- **实时图表**：频谱瀑布图或柱状图显示

**技术优势**：
- **SX1302 硬件支持**：利用 SX1262 辅助芯片实现高灵敏度扫描
- **快速扫描**：全频段扫描时间 < 10 秒
- **精准定位**：频率分辨率 125kHz
- **自动优化**：根据扫描结果自动推荐信道配置
...
**功能位置**：`www/lora status` 目录

**核心功能**：
- **射频状态**：实时显示两个 LoRa 模块的工作状态、频率、扩频因子、带宽
- **上下行统计**：接收包数、丢包数、CRC 错误、发送成功率
- **信号质量分析**：RSSI、SNR 分布图
- **网关性能指标**：CPU 使用率、内存占用、网络流量

**技术优势**：
- **实时性高**：数据刷新间隔 5 秒
- **多维度分析**：从 RF 层、协议层、系统层全面监控
- **异常告警**：丢包率、CRC 错误率超阈值时自动告警
- **性能基准**：提供网络健康评分
...
**功能位置**：`www/ns history` 目录

**核心功能**：
- **完整历史记录**：记录所有 LoRaWAN 设备的入网、上行、下行、确认事件
- **多条件筛选**：按设备 EUI、事件类型、时间范围筛选
- **详细信息查看**：LoRaWAN 帧详情、原始 Payload、解码后的 JSON 数据
- **Excel 导出**：导出筛选结果为 Excel 表格
...
**功能位置**：`www/ns status` 目录

**核心功能**：
- **实时数据展示**：显示所有在线设备的最新上行数据
- **批量设备管理**：
  - **批量删除**：选择多个设备一键删除
  - **Excel 批量导入**：上传 Excel 文件批量添加设备
  - **设备导出**：导出当前设备列表为 Excel
- **chirpstack 快速访问**：一键跳转到 chirpstack Web 界面

**技术优势**：
- **高效批量操作**：Excel 导入功能支持一次性添加数百个设备
- **零停机迁移**：设备数据导出/导入，便于网关更换
- **实时性强**：WebSocket 推送，设备数据延迟 < 2 秒
- **操作简便**：无需登录 chirpstack

---

### 12. 安全远程协助

**功能位置**：`www/remote assist` 目录

**核心功能**：
- **加密 VPN 隧道**：基于 WireGuard 或 OpenVPN 技术
- **按需激活**：用户通过 Web 界面一键激活远程协助
- **会话管理**：显示远程协助状态、连接时长
- **自动断开**：协助完成后自动断开 VPN
- **访问权限控制**：仅限访问网关配置和日志
- **审计日志**：记录所有远程协助会话
...
**技术优势**：
- **安全性高**：端到端加密（AES-256），双向认证
- **无需额外软件**：不依赖 TeamViewer 等第三方工具
- **直接访问**：技术支持可直接访问网关 SSH、Web 界面
- **隐私保护**：技术支持无法查看 LoRaWAN 设备的业务数据
